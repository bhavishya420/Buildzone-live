-- Fix BNPL Credit Scoring Tier Logic
-- This migration corrects the tier calculation logic in the calculate_credit_eligibility function

-- Drop existing function
DROP FUNCTION IF EXISTS public.calculate_credit_eligibility(uuid);

-- Create corrected function with proper tier logic
...
BEGIN
    ...
    -- Determine tier based on CREDIT SCORE (FIXED LOGIC)
    IF credit_score >= 0 AND credit_score <= 100 THEN
        tier_value := 'Silver';
    END IF;
    
    IF credit_score >= 101 AND credit_score <= 250 THEN
        tier_value := 'Gold';
    END IF;
    
    IF credit_score >= 251 THEN
        tier_value := 'Platinum';
    END IF;
    ...
    -- Return result JSON with required fields
...
COMMENT ON FUNCTION public.calculate_credit_eligibility(uuid) IS 'Calculate BNPL credit eligibility with corrected tier logic based on credit score: Silver (0-100), Gold (101-250), Platinum (251+)';